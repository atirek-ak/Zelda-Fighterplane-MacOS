...
Bomb::Bomb(float x, float y, float z) {
    ...
    for(int i=0;i<100;i++){
        ...
        vertex_buffer_data[9*i+5]=0.3*sin(angle);
        vertex_buffer_data[9*i+6]=0.3*cos(angle+rot_angle);
        vertex_buffer_data[9*i+7]=0.0;
        vertex_buffer_data[9*i+8]=0.3*sin(angle+rot_angle);
    ...
    this->object = create3DObject(GL_TRIANGLES, 100*3, vertex_buffer_data, COLOR_DARK_OLIVE_GREEN, GL_FILL);
    this->object1 = create3DObject(GL_TRIANGLES, 200*3, vertex_buffer_data2, COLOR_DARK_OLIVE_GREEN, GL_FILL);
    this->object3 = create3DObject(GL_TRIANGLES, 4*3, vertex_buffer_data3, COLOR_DARK_OLIVE_GREEN, GL_FILL);
}
